// // app/(vendor)/vendor/integrations/page.tsx
// 'use client';
//
...
// // Düzeltme: 'Sync' yerine 'RefreshCw' kullanıldı.
// import { CheckCircle, XCircle, Clock, Save, RefreshCw, KeyRound, Lock, Zap } from 'lucide-react';
//
...
// }
//
// // TanStack Query Hook'ları (Değişmedi)
...
//             return data;
//         },
//         initialData: [], // Yüklenme anında boş dizi döner
//     });
// };
//
...
//         },
//         onError: (error) => {
//             alert(`Kaydetme hatası: ${(error as any).response?.data?.message || 'Bilinmeyen Hata'}`);
//         }
//     });
// };
//
...
//     const { mutate: saveIntegration, isPending } = useSaveIntegration();
//
//     // Varsayılan Trendyol yapılandırması
...
//     };
//
//     // Form durumunu tutmak
//     const [trendyolConfig, setTrendyolConfig] = useState(defaultTrendyolConfig);
//
//     // Yüklenme anında API'den gelen veriyi forma yansıtma
...
//                 sellerId: trConfig.sellerId || '', // sellerId'yi doldur
//                 // Not: apiKey ve apiSecret boş kalır (güvenlik)
//             });
//         }
//     }
//
...
//             sellerId: trendyolConfig.sellerId, // Yeni eklenen alan
...
//                         const isConfigured = !!i;
//                         const isEnabled = i?.isEnabled || false;
//
//                         return (
//                             <div key={market.key} className={`p-6 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-[1.02] ${
//                                 isEnabled ? 'bg-white border-2 border-green-400 dark:bg-zinc-800' :
//                                     isConfigured ? 'bg-yellow-50/50 border border-yellow-300 dark:bg-zinc-700' :
//                                         'bg-gray-50 border border-gray-200 dark:bg-zinc-700'
//                             }`}>
//                                 <div className="flex items-center space-x-4 mb-4">
//                                     <img src={market.logo} alt={`${market.name} Logo`} className="rounded-full w-10 h-10 object-cover" />
//                                     <h3 className="font-bold text-xl text-gray-800 dark:text-zinc-100">{market.name}</h3>
//                                 </div>
//
//                                 <p className={`text-sm font-medium ${isEnabled ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-400'}`}>
//                                     {isEnabled ? 'Durum: Aktif ve Senkronize' : 'Durum: Pasif / Yapılandırılmadı'}
//                                 </p>
//
//                                 <div className="mt-4 pt-3 border-t border-dashed border-gray-300 dark:border-zinc-600 text-xs text-gray-500 dark:text-zinc-400 space-y-1">
//                                     <div className="flex justify-between items-center">
//                                         <span>Ürün Sayısı:</span>
//                                         <span className="font-semibold text-gray-700 dark:text-zinc-200">{i?.totalProducts || 0}</span>
//                                     </div>
//                                     <div className="flex justify-between items-center">
//                                         <span>Son Senkronizasyon:</span>
//                                         <div className="flex items-center space-x-1">
//                                             <StatusIcon status={i?.lastSyncStatus || 'IDLE'} />
//                                             <span>{i?.lastSyncAt ? new Date(i.lastSyncAt).toLocaleDateString() : 'Hiç Yok'}</span>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
...
//
//             {/* Trendyol Ayar Formu */}
//             <section className="max-w-xl mx-auto p-8 border rounded-2xl shadow-2xl bg-white dark:bg-zinc-800 dark:border-zinc-700">
//                 <h2 className="text-3xl font-bold mb-6 text-indigo-700 dark:text-indigo-400 flex items-center space-x-2">
...
//                     {/* Satıcı ID */}
//                     <div>
//                         <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-zinc-300">Trendyol Satıcı ID (Vendor ID)</label>
...
//                                 className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white transition"
...
//                     {/* API Key */}
//                     <div>
//                         <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-zinc-300">API Anahtarı</label>
//                         <div className="relative">
...
//                                 className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white transition"
//                                 placeholder="Yeni API Key'i giriniz (Boş bırakılırsa değişmez)"
//                             />
//                         </div>
//                     </div>
//
//                     {/* API Secret */}
//                     <div>
//                         <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-zinc-300">API Secret</label>
//                         <div className="relative">
//                             <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
//                             <input
//                                 type="password"
...
//                                 className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white transition"
//                                 placeholder="Yeni API Secret'ı giriniz (Boş bırakılırsa değişmez)"
//                             />
//                         </div>
//                     </div>
//
...
//                             id="trendyol-enabled"
//                             className="h-5 w-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-zinc-600 dark:border-zinc-500"
//                         />
//                         <label htmlFor="trendyol-enabled" className="text-base font-medium text-gray-700 dark:text-zinc-300">Entegrasyonu Aktif Et</label>
//                     </div>
//
//                     {/* Kaydet Butonu */}
//                     <button
//                         type="submit"
//                         disabled={isPending}
//                         className="w-full py-3 px-4 flex items-center justify-center space-x-2 rounded-xl shadow-lg text-lg font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 focus:outline-none focus:ring-4 focus:ring-indigo-500 focus:ring-opacity-50 disabled:bg-indigo-400"
//                     >
//                         <Save className="w-5 h-5" />
//                         <span>{isPending ? 'Kaydediliyor...' : 'Ayarları Kaydet ve Bağlan'}</span>
//                     </button>
//                 </form>
//             </section>
//         </div>
...
const useSyncProducts = () => {
    ...
    return useMutation({
        mutationFn: async (marketplace: MarketplaceName) => {
            // Senkronizasyon API rotasını çağırıyoruz
            const { data } = await axios.post('/api/vendor/integrations/sync-products', { marketplace });
            return data;
        },
        onSuccess: () => {
            // Senkronizasyon başarılıysa entegrasyon listesini yenile
            queryClient.invalidateQueries({ queryKey: ['vendor', 'integrations'] });
            // Bildirim olarak modal yerine alert kullanıyoruz
